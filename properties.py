...
class GasProperties:
    ...
    def __init__(self, mech_file=config.MECH_FILE, use_rk=config.USE_RK_EOS):
        ...
        # 元素インデックスと原子量を取得して、計算を高速化するために保持しておく
        self.C_element_idx = self.gas.element_index('C')
        self.H_element_idx = self.gas.element_index('H')
        self.atomic_weights = self.gas.atomic_weights
        self.species_n_atoms = np.zeros((self.nsp, self.gas.n_elements))
        for i in range(self.nsp):
            for j in range(self.gas.n_elements):
                self.species_n_atoms[i, j] = self.gas.n_atoms(i, j)
        print("Elemental composition data for all species has been cached.")
    ...
    def get_diffusion_coeffs(self, T, P, Y, option=config.DIFFUSION_OPTION):
         ...
         if option == 'Le=1':
             ...
             try:
                 ...
                 Dk_mix_mole = self.gas.mix_diff_coeffs * config.DIFFUSION_COEFFICIENT_SCALE
    ...
    def get_elemental_fuel_mass_fraction(self, Y_k):
        """
        化学種質量分率ベクトル Y_k から、燃料由来元素(CとH)の
        合計質量分率を計算する。
        Y_fuel = Σ_i ( Y_i * (m_C,i + m_H,i) / m_i )
        ここで m_C,i は化学種iに含まれるC原子の全質量。
        これは、混合物中のC原子とH原子の全質量分率の和と等価。
        """
        if Y_k.ndim == 1: # 単一の点の場合
            Y_k = Y_k[:, np.newaxis] # 2次元配列に変換

        n_points = Y_k.shape[1]
        elemental_frac = np.zeros(n_points)

        # C原子の質量分率を計算
        mass_frac_C = np.dot(
            (self.species_n_atoms[:, self.C_element_idx] * self.atomic_weights[self.C_element_idx] / self.molecular_weights).T,
            Y_k
        )
        # H原子の質量分率を計算
        mass_frac_H = np.dot(
            (self.species_n_atoms[:, self.H_element_idx] * self.atomic_weights[self.H_element_idx] / self.molecular_weights).T,
            Y_k
        )

        elemental_frac = mass_frac_C + mass_frac_H
        
        # 入力が単一の点だった場合は、スカラー値を返す
        return elemental_frac.item() if n_points == 1 else elemental_frac
